...
class Chronos2Dataset(IterableDataset):
    ...
    def __init__(
        ...
        aug_jitter_sigma: float = 0.03,
        aug_scaling_sigma: float = 0.1,
        aug_mag_warp_sigma: float = 0.2,
        aug_mag_warp_knots: int = 4,
        aug_prob: float = 0.5,
        covariate_dropout_prob: float = 0.0,
    ) -> None:
        super().__init__()
        ...
        self.augmentation = augmentation and (mode == DatasetMode.TRAIN)
        self.aug_jitter_sigma = aug_jitter_sigma
        self.aug_scaling_sigma = aug_scaling_sigma
        self.aug_mag_warp_sigma = aug_mag_warp_sigma
        self.aug_mag_warp_knots = aug_mag_warp_knots
        self.aug_prob = aug_prob
        self.covariate_dropout_prob = covariate_dropout_prob if (mode == DatasetMode.TRAIN) else 0.0
    ...
    def _apply_jitter(self, x: torch.Tensor, n_targets: int, std_val: torch.Tensor) -> torch.Tensor:
        """
        Apply jitter (Gaussian noise) to target series only.

        Args:
            x: tensor of shape (n_targets + n_covariates, seq_len)
            n_targets: number of target variables (first n_targets rows)
            std_val: standard deviation of the target series

        Returns:
            Jittered tensor with same shape
        """
        x = x.clone()
        target_part = x[:n_targets, :]
        finite_mask = torch.isfinite(target_part)

        if np.random.random() < self.aug_prob and std_val > 0:
            noise = torch.randn_like(target_part) * self.aug_jitter_sigma * std_val
            noise[~finite_mask] = 0  # Don't add noise to NaN values
            target_part = target_part + noise
            # Ensure non-negative (for solar power data)
            target_part = torch.clamp(target_part, min=0)
            x[:n_targets, :] = target_part

        return x

    def _apply_scaling(self, x: torch.Tensor, n_targets: int, scale_factor: float) -> torch.Tensor:
        """
        Apply scaling to target series only.
        ...
        x = x.clone()
        target_part = x[:n_targets, :]
        target_part = target_part * scale_factor
        # Ensure non-negative (for solar power data)
        target_part = torch.clamp(target_part, min=0)
        x[:n_targets, :] = target_part
        return x
    ...
    def _get_scale_factor(self) -> float | None:
        """Generate a random scale factor if augmentation is triggered."""
        if np.random.random() < self.aug_prob:
            scale_factor = np.random.normal(1.0, self.aug_scaling_sigma)
            scale_factor = np.clip(scale_factor, 0.8, 1.2)  # Limit to 80%-120%
            return float(scale_factor)
        return None

    def _generate_mag_warp_curve(self, length: int) -> torch.Tensor | None:
        """
        Generate a smooth magnitude warping curve using cubic spline interpolation.

        Args:
            length: total length of the curve (context_length + prediction_length)

        Returns:
            A tensor of shape (length,) with smooth warping factors around 1.0,
            or None if augmentation is not triggered.
        """
        if np.random.random() >= self.aug_prob:
            return None

        # Generate random knot values around 1.0
        knot_values = np.random.normal(1.0, self.aug_mag_warp_sigma, self.aug_mag_warp_knots + 2)
        knot_values = np.clip(knot_values, 0.5, 1.5)  # Limit warping range

        # Knot positions (evenly spaced including endpoints)
        knot_positions = np.linspace(0, length - 1, self.aug_mag_warp_knots + 2)

        # Cubic spline interpolation
        from scipy.interpolate import CubicSpline
        spline = CubicSpline(knot_positions, knot_values)
        warp_curve = spline(np.arange(length))

        # Ensure positive values
        warp_curve = np.clip(warp_curve, 0.5, 1.5)

        return torch.from_numpy(warp_curve).float()

    def _apply_magnitude_warping(
        self,
        ...
        """
        Apply magnitude warping to both context and target using a smooth curve.

        Args:
            context: tensor of shape (n_targets + n_covariates, context_len)
            target: tensor of shape (n_targets + n_covariates, prediction_len)
            n_targets: number of target variables
            warp_curve: tensor of shape (context_len + prediction_len,)

        Returns:
            Tuple of (warped_context, warped_target)
        """
        ...
        # Split curve into context and target parts
        context_curve = warp_curve[:context_len]
        target_curve = warp_curve[context_len:]

        # Apply warping to target series only (first n_targets rows)
        context[:n_targets, :] = context[:n_targets, :] * context_curve.unsqueeze(0)
        target[:n_targets, :] = target[:n_targets, :] * target_curve.unsqueeze(0)

        # Ensure non-negative
        context[:n_targets, :] = torch.clamp(context[:n_targets, :], min=0)
        target[:n_targets, :] = torch.clamp(target[:n_targets, :], min=0)
    ...
    def _construct_slice(self, task_idx: int) -> tuple[torch.Tensor, torch.Tensor | None, torch.Tensor, int]:
        ...
        if slice_idx >= self.context_length:
            ...
            task_context = task_past_tensor[:, slice_idx - self.context_length : slice_idx].clone()
        else:
            task_context = task_past_tensor[:, :slice_idx].clone()
        ...
        # - Jitter: only applied to context (adds noise)
        # - Scaling: applied to both context and target (preserves relative relationship)
        # - Magnitude Warping: applied to both context and target with smooth curve
        if self.augmentation and self.mode == DatasetMode.TRAIN:
            # Compute std from context for jitter
            target_part = task_context[:task_n_targets, :]
            finite_mask = torch.isfinite(target_part)
            std_val = target_part[finite_mask].std() if finite_mask.any() else torch.tensor(1.0)

            # Apply jitter to context only
            task_context = self._apply_jitter(task_context, task_n_targets, std_val)

            # Apply same scaling to both context and target
            scale_factor = self._get_scale_factor()
            if scale_factor is not None:
                task_context = self._apply_scaling(task_context, task_n_targets, scale_factor)
                task_future_target = self._apply_scaling(task_future_target, task_n_targets, scale_factor)

            # Apply magnitude warping to both context and target
            total_len = task_context.shape[-1] + self.prediction_length
            warp_curve = self._generate_mag_warp_curve(total_len)
            if warp_curve is not None:
                task_context, task_future_target = self._apply_magnitude_warping(
                    task_context, task_future_target, task_n_targets, warp_curve
                )
        ...
        # Covariate dropout: randomly mask all covariates to force model to learn from target history
        if self.covariate_dropout_prob > 0 and self.mode == DatasetMode.TRAIN and task_n_covariates > 0:
            if np.random.random() < self.covariate_dropout_prob:
                task_context[task_n_targets:, :] = torch.nan
                task_future_covariates[:, :] = torch.nan
    ...
    def convert_inputs(
        ...
        return cls(
            ...
            aug_scaling_sigma=aug_scaling_sigma,
            aug_mag_warp_sigma=aug_mag_warp_sigma,
            aug_mag_warp_knots=aug_mag_warp_knots,
            aug_prob=aug_prob,
            covariate_dropout_prob=covariate_dropout_prob,
        )
